...
namespace Server
{
	...
	class ServiceLogic : IService 
	{
		...
		private static readonly ServiceLogic _instance = new ServiceLogic();

		/// <summary>
		/// Get's the same instance
		/// </summary>
		/// <returns>existing instance</returns>
		public static ServiceLogic GetClientLogic()
        {
			return _instance;
        }
		...
		public double RemoveGasAmount(double amount)
		{
			try
            {
				...
				log.Info($"Gas station has:{tank} l");
				return tank;
			}
            catch (Exception)
            {
